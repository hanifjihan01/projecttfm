'use client';

import { motion } from 'framer-motion';

export function PalmveinParameter() {
  const parameters = [
    {
      label: 'System',
      value:
        'High-performance SOC chip-based facial recognition system running on Linux OS.',
    },
    {
      label: 'Camera & Lens',
      value:
        '2.0MP camera with 4.3mm lens, capable of face recognition in under 1 second with ≥99.7% accuracy.',
    },
    {
      label: 'Recognition Range & Capacity',
      value:
        'Recognition distance: 0.3–2 meters, supports up to 5,000 stored faces.',
    },
    {
      label: 'Display',
      value: '5-inch IPS LCD touchscreen with 1280x720 resolution.',
    },
    {
      label: 'Connectivity',
      value: 'Supports TCP/IP, Wi-Fi, USB, RS485, and Wiegand Output.',
    },
    {
      label: 'Built-in Features',
      value:
        'Built-in speaker, white LED, IR light, and upgrade & recovery buttons.',
    },
    {
      label: 'Design & Dimensions',
      value: 'Wall-mountable design, 154x154x20 mm, weight 730g.',
    },
    {
      label: 'Power Supply',
      value: 'DC 12V 2A.',
    },
    {
      label: 'Operating Conditions',
      value: 'Temperature: -20℃ to 60℃, Humidity: 10–90%.',
    },
  ];

  return (
    <section className="w-full bg-accent-900 px-4 py-10 sm:px-6 md:px-10 lg:px-24">
      <div className="mx-auto w-full max-w-full md:max-w-[700px] lg:max-w-[900px] xl:max-w-[1000px]">
        <motion.h2
          initial={{ opacity: 0, y: -10 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4 }}
          viewport={{ once: true }}
          className="mb-6 text-lg font-semibold text-white sm:text-xl md:text-2xl"
        >
          Parameters
        </motion.h2>

        <div className="overflow-hidden rounded-xl border border-white/10">
          {parameters.map((item, index) => (
            <div
              key={index}
              className={`flex flex-col gap-1 px-4 py-4 text-sm sm:flex-row sm:items-start sm:gap-4 sm:text-base ${
                index % 2 === 0
                  ? 'bg-white/10 hover:bg-white/10'
                  : 'bg-white/5 hover:bg-white/20'
              } transition-colors duration-200`}
            >
              <div className="w-full font-medium text-gray-300 sm:w-1/3 md:w-1/4">
                {item.label}
              </div>
              <div className="w-full text-gray-100 sm:w-2/3 md:w-3/4">
                {item.value}
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
